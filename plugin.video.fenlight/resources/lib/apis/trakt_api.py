...
def get_trakt(params):
    ...
    if not result:
        return ([], 0) if params.get('pagination', True) else None
...
def call_trakt(path, params={}, data=None, is_delete=False, with_auth=True, method=None, pagination=False, page_no=1):
    def send_query():
        resp = None
        if with_auth:
            token = get_setting('trakt.token')  # Standardized key
    ...
    if status_code == 401:
        ...
        if 'Retry-After' in headers:
            kodi_utils.sleep(1000 * int(headers['Retry-After']))
            response = send_query()
    ...
    if pagination: 
        page_count = headers.get('X-Pagination-Page-Count', 1)
...
def trakt_refresh_token():
    try:
        ...
        refresh_token = get_setting('trakt.refresh')  # Fixed key reference
        data = {        
            'client_id': CLIENT_ID, 
...
def trakt_revoke_authentication(dummy=''):
    token = get_setting('trakt.token')  # Fixed key reference
